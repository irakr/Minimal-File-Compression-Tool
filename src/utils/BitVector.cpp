...
{
    if (pos >= size)
        throw (new std::overflow_error("BitVector::toggleBit(): Bit out of range."));
...
{
    if (pos >= size)
        throw (new std::overflow_error("BitVector::getBit(): Bit out of range."));
    uint8_t field = bytes[pos / UINT8_LENGTH];
    return field & (1 << (pos % UINT8_LENGTH));
}

// NOTE: The way this operator function shows the bit pattern is just the opposite
// of its actual storage in 'bytes'.
 ...
 * Added by: Irak Rigia
 *
 * This function returns the C like character array from the internal
 * byte vector.
 */
const unsigned char* BitVector :: toCharArray() const
{
    return bytes.data();
}
...
void BitVector::stringToBitVector(const char *str)
{
    ...
    for (int i = 0; i < strSize; i++)
    {
        if (str[i] == '1')
            ...
            throw (new std::overflow_error("BitVector::stringToBitVector(): str must represent a binary number"));
    }
}
...
int BitVector::computeHammingDistance(const BitVector& u) const
{
    if (getSize() != u.getSize())
        throw (new std::overflow_error("BitVector::computeHammingDistance(): You can't compute Hamming distance between two vectors with different sizes"));
